...
```bash
dk bash  # starts bash inside "dolceria" container
...
* `dkup` uses docker-compose to start 3 containers: postgres, nginx, and dolceria.
...
* Once `dkup` is running, `dk <command>` will run `<command>` inside the `dolceria` container. So `dk bash` will get you "logged in" as root inside that container. Once inside, you need to run Django's `manage.py` commands to initialize the database properly.
* The dolceria container runs 3 services:
 * django on port 8000
...
```bash
mkvirtualenv dolceria -p python3  # creates a python3 virtualenv
